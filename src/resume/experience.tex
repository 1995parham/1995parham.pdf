...
\cventry{Feb 2022--Current}{Distinguished Engineer (Cloud Platform)}{Snapp!}{Tehran, Iran}{}{
	\begin{itemize}
		\item Provide CRDs and tailor-made operator to maintain authentication and authorization over ArgoCD
		\item Provide in-house ChatBot using Ollama, LiteLLM and OpenWebUI.
		\item Led the design and implementation of a scalable, microservice-based architecture for a delivery industry application,
		      ...
		      more that 300k messages per second over different quality of services.
		\item Setup \textbf{Machine learning pipelines} using classical and neural network algorithms on \textbf{Kubernetes}
          based on \textbf{KServe} and \textbf{KNative} to reduce resource usage and increase the data gathering by 10\%.
		\item Lunch Kubernetes pods periodically using \httpslink[\textbf{Airflow}]{airflow.apache.org} for data processing to replace old cron job.
          Airflow provides monitoring, re-run and other features which improved the platform reliability.
		...
		\item Write data ingestion pipelines using \textbf{Apache Spark} and \textbf{Apache Beam} in Java which improved
          pipeline efficiency and reduced resource usage.
		\item Deploy Kafka on Kubernetes using \httpslink[\textbf{Strimzi}]{https://strimzi.io/}.
		\item Mentoring teams for effective use of NATS and other messaging systems by holding sessions and reviewing their code.
    \item 5\% improve in Central Messaging Queue uptime by creating a unified SDK for teams to use it.
	\end{itemize}
}

\cventry{Feb 2022--Current}{Senior Infrastructure Engineer}{Nobitex}{Tehran, Iran}{}{
	\begin{itemize}
...
\cventry{Oct 2020--Feb 2022}{Senior Software Engineer / Team Lead (Dispatching Team)}{Snapp!}{Tehran, Iran}{}{
	\begin{itemize}
    ...
    \item Implement polling solution which handles 300k http calls per second which worked as a backup solution
          to keep drivers avaiable in case of message-passing solution failed.
    ...
    \item Implement in-application call between driver and passenger based on \httpslink[\textbf{Pion}]{github.com/pion}
          which replaced number masking solution and reduced the company costs about 50\%.
	\end{itemize}
}
